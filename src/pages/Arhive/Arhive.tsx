...
const Arhive: React.FC = () => {
  ...
  const scrollHandler = () => {
    if (document.documentElement.scrollHeight - (document.documentElement.scrollTop + window.innerHeight) < 100 && nextPage) {
      setFetching(true);
    }
  ...
  useEffect(() => {
    if (fetching) {      
      axios
      .get<ISeasonApiResponse>(`${import.meta.env.VITE_API_URL}seasons/?_limit=10&page=${page}`)
      .then(res => {
  ...
  const handleSortByWinner = () => {
    ...
    newSeasons.sort((a, b) => a.winner ? a.winner.localeCompare(b.winner ? b.winner : '') : ''.localeCompare(b.winner ? b.winner : ''));
