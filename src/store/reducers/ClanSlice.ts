...
const clanSlice = createSlice({
    ...
    reducers: {
        ...
        setPlayerWins: (state, action: PayloadAction<{ playerWins: number, index: number}>) => {
            const trueIndex = state.players.findIndex((p) => p.id === action.payload.index)
            if (trueIndex !== -1) {
                state.players[trueIndex].wins = action.payload.playerWins
            }
        },
        setPlayerGames: (state, action: PayloadAction<{ playerGames: number, index: number}>) => {
            const trueIndex = state.players.findIndex((p) => p.id === action.payload.index)
            if (trueIndex !== -1) {
...
export const { setClan, setLoading, updateClanField, setError, setPlayers, setPlayerGames, setPlayerWins } = clanSlice.actions;
