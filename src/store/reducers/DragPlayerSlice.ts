  ...
  reducers: {
    ...
    setDroppedPlayer: (state, action: PayloadAction<(IPlayer | null)[]>) => {
      action.payload.forEach((player) => {
        const index = state.playerDropped.findIndex(
          (p) => p?.id === player?.id
        );
        if (index === -1) {
    ...
    setDraggable: (state, action: PayloadAction<[number, boolean]>) => {
      const index = state.draggable.findIndex(
        (p) => p[0] === action.payload[0]
      );
      if (index === -1) {
...
export const selectDragPlayer = (state: RootState) =>
  state.dragPlayer.playerDragged;
export const selectDroppedPlayer = (state: RootState) =>
  state.dragPlayer.playerDropped;
