...
const groupsSlice = createSlice({
  ...
  reducers: {
    ...
    updateGroupTeams: (
      state,
      action: PayloadAction<{ groupId: number; team: IClan }>
    ) => {
    ...
    addGroup: (state) => {
      ...
      if (state.groups.length > 0) {
        newGroupMark = String.fromCharCode(
          state.groups[state.groups.length - 1].groupMark.charCodeAt(0) + 1
        );
      } else {
        newGroupMark = "A";
      }
...
export const selectUndistributedTeams = (state: RootState) =>
  state.groupsManager.undistributedTeams;

export const { setGroups, setUndistributedTeams, updateGroupTeams, addGroup } =
  groupsSlice.actions;
