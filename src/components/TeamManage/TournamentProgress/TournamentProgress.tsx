...
const TournamentProgress: FC = () => {
  ...
  const tournamentsWebSocketFunc = () => {
    const tournamentsWebSocket = new WebSocket(
      `${import.meta.env.VITE_SERVER_WS_URL}tournament_status/`
    );
    ...
    if (tournamentsData) {
      tournamentsData.forEach((tournament) => {
        if (tournament.isFinished === true) {
          ...
          if (unstartedTournaments.indexOf(tournament.id) === -1)
            dispatch(setUnstartedTournaments(tournament.id));
        }
      });
    }
  }, [tournamentsData]);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (tournamentsData) {
        tournamentsData.forEach((tournament) => {
          if (
            moment(tournament.startTime).isBefore(new Date()) &&
            tournament.isFinished === false &&
            unstartedTournaments.indexOf(tournament.id) !== -1
          ) {
  ...
  return (
    <div className={classes.tournamentProgressBlock}>
      {tournamentsData &&
        tournamentsData.findIndex(
          (tournamet) => tournamet.isFinished === false
        ) === -1 &&
        tournamentsData.findIndex(
          (tournament) => tournament.tournamentInGroup === false
        ) === -1 && (
          <h3>
            <FormattedMessage id="groupStageCompleteMessage" />
          </h3>
        )}
      {tournamentsData &&
        ...
        Object.keys(matches).length > 0 && (
          <div className={classes.tournamentProgress}>
            <h2>
              <FormattedMessage id="currentTournamentsTitle" />
            </h2>
            {matches && Object.keys(matches).length === 0 && <Loader7x />}
            {matches &&
              Object.keys(matches).map((key) => {
                const tournament = tournamentsData?.find(
                  (tournament) => tournament.id === parseInt(key)
                );
                if (tournament?.isFinished) {
                  return null;
                }
                return (
                  <div className={classes.tournament} key={key}>
                    {tournament?.teamInTournament === 1 ? (
                      <div className={classes.tournamentHeader}>
                        ...
                        <button
                          ...
                          onClick={() => {
                            matchesWebSocketsRef.current[parseInt(key)].send(
                              JSON.stringify({
                                action: "create",
                              })
                            );
                    ...
                    <div className={classes.tourBlock}>
                      {Object.keys(matches[parseInt(key)]).map((key2) => {
                        return (
                          <div className={classes.match} key={key2}>
                              ...
                              <select
                                ...
                                onChange={(event) => {
                                  ...
                                  const player = players.find(
                                    (player) =>
                                      player.id === parseInt(event.target.value)
                                  );
                                  ...
                                  const playerGames = player
                                    ? player.total_games
                                    : 0;
                                  const oldPlayerGames = oldPlayer
                                    ? oldPlayer.total_games
                                    : 0;
                                  dispatch(
                                    setPlayerGames({
                                      ...
                                      >
                                        {
                                          myTeam?.players.find(
                                            (teamPlayer) =>
                                              teamPlayer.id === player.player
                                          )?.username
                                        }
                                      </option>
                                    );
                                  })}
                                {tournament?.teamInTournament === 2 &&
                                  tournament?.opponent.players.map((player) => {
                                    return (
                            ...
                            <div className={classes.matchLineBottom}>
                              ...
                              >
                                <option value="0" disabled>
                                  <FormattedMessage id="selectWinnerLabel" />
                                </option>
                                {tournament?.teamInTournament === 1 && (
                                  <option
                                    value={
                                ...
                                {tournament?.teamInTournament === 1 && (
                                  ...
                                  >
                                    {
                                      tournament?.opponent.players.find(
                                        (player) =>
                                          player.id ===
                                          matches[parseInt(key)][parseInt(key2)]
                                            .player_two
                      ...
                      <div className={classes.buttonBlock}>
                        {tournament?.askForFinished &&
                          !tournament?.askedTeam && (
      ...
      <div className={classes.notRunning}>
        <div className={classes.notRunningBlock}>
          {tournamentsData &&
            tournamentsData.length > 0 &&
            tournamentsData.findIndex(
              (tournament) => tournament.isFinished === true
            ) !== -1 && (
              <h2 className={classes.finishedTournamentsTitle}>
                <FormattedMessage id="finishedTournamentsTitle" />
              </h2>
            )}
          {tournamentsData &&
            tournamentsData.map((tournament) => {
              if (tournament.isFinished === true) {
                return (
                  <div
                    className={classes.finishedTournament}
                    key={tournament.id}
                  >
                    {tournament.teamInTournament === 1 && (
                      <div
                        className={classes.finishedTournamentHeader}
                        style={
                      ...
                      >
                        ...
                        <h3>
                          ...
                          tournament.teamOneWins === 0 &&
                          tournament.winner &&
                          tournament.winner === tournament.opponent.id
                            ? "TL"
                            : null}
                          :
                          {tournament.teamOneWins !== undefined &&
                          (tournament.teamOneWins !== 0 ||
                            tournament.teamTwoWins !== 0)
                            ? tournament.teamOneWins
                            : null}
                    ...
                    {tournament.matches && tournament.matches.length > 0 && (
                      <div
                        className={classes.finishedTournamentMatchesContainer}
                      >
                        <button
                          className={`${classes.matchesResults} ${
                            matchShowed[tournament.id] ? classes.show : ""
                          }`}
                          onClick={() => {
                            dispatch(
                              setMatchShowed({
                        ...
                        >
                          <h3 className={classes.matchesResultsTitle}>
                            <FormattedMessage id="matchResultsTitle" />{" "}
                          </h3>
                          <div
                            className={`${classes.arrow} ${
                              matchShowed[tournament.id] ? classes.rotate : ""
                            }`}
                          >
                            {" "}
                            &gt;{" "}
                        ...
                        <div
                          className={`${classes.finishedTournamentMatches} ${
                            matchShowed[tournament.id] ? "" : classes.hide
                          }`}
                        >
                          {tournament.matches &&
                            tournament.matches.map((match) => {
                              return (
                                <div
                                  key={match.id}
                                  className={`${
                                    classes.finishedTournamentMatchContainer
                                  } ${
                                    matchShowed[tournament.id]
                                      ? ""
                                      : classes.hiden
                                  }`}
                                >
                                  <div
                                    className={classes.finishedTournamentMatch}
                                  >
                                    <h3
                                      className={`${
                                        match.winner === match.player_one
                                          ? classes.winner
                                          : classes.participant
                                      } ${classes.player} ${classes.playerOne}`}
                                    >
                                      {myTeam?.players.find(
                                        (player) =>
                                          player.id === match.player_one
                                      )?.username ||
                                        tournament.opponent.players.find(
                                          (player) =>
                                            player.id === match.player_one
        ...
        <div className={classes.notRunningBlock}>
          {tournamentsData &&
            ...
            tournamentsData.findIndex(
              (tournament) =>
                moment(tournament.startTime) > moment() &&
                tournament.isFinished !== true
            ) !== -1 && (
              <h2 className={classes.finishedTournamentsTitle}>
                <FormattedMessage id="upcomingTournamentsTitle" />
              </h2>
            )}
          {tournamentsData &&
            tournamentsData.map((tournament) => {
              if (
                ...
                return (
                  <div
                    className={classes.upcomingTournament}
                    key={tournament.id}
                  >
                    ...
                    <h3>
                      <FormattedMessage id="startTimeLabel" />:{" "}
                    ...
                    >
                      <FormattedMessage id="startEarlyLabel" />
                    </Button7x>
                  </div>
                );
              }
