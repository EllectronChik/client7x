...
const TournamentCard: FC<TournamentCardProps> = ({ ...props }) => {
  ...
  const [timeSuggestion, setTimeSuggestion] = useState<string | null>(
    props.tournament.timeSuggested
  );
  const [sendChangeTimeSuggestion] =
    TournamentApi.useSetTimeSuggestionMutation();
  const [acceptTimeSuggestion] =
    TournamentApi.useAcceptTimeSuggestionMutation();
  ...
  return (
    <div
      className={`${props.className ? props.className : ""} ${
        classes.tournamentCard
      } ${askForChange ? classes.askForChange : ""} ${
        timeSuggestion ? classes.timeSuggested : ""
      }`}
    >
      <div className={classes.opponent}>
        <div>
          ...
          <div className={classes.opponentLeft}>
            <img
              className={classes.opponentLogo}
              src={teamDefault}
              alt=""
              onLoad={(e) => {
                e.currentTarget.src = `${import.meta.env.VITE_SERVER_URL}${
                  props.tournament.opponent.logo
                }`;
              }}
            />
            <div>
        ...
        <div className={classes.opponentRight}>
          ...
          <Tooltip
            ...
            id={`changeTime_${props.tournament.id}`}
          >
            <h3>
              You can send a request to your opponent to change the time, <br />{" "}
              they will be able to accept the time change or offer an
              alternative{" "}
            </h3>
          </Tooltip>
          <div
            className={classes.changeTime}
            data-tooltip-id={`changeTime_${props.tournament.id}`}
            data-tooltip-place="right"
          >
            <Button7x
              className={classes.button}
              onClick={() => setAskForChange(askForChange ? false : true)}
            >
              Change time
            </Button7x>
          </div>
      ...
      {askForChange && (
        <div className={classes.inputContainer}>
          <input
            value={moment(inputValue).format("YYYY-MM-DDTHH:mm")}
            onChange={(e) => {
              ...
              changeStartTimeTimeout = setTimeout(() => {
                sendChangeTimeSuggestion({
                  id: props.tournament.id,
                  timeSuggestion: new Date(e.target.value).toISOString(),
                  token: cookies.token,
                });
      ...
      <div>
        {timeSuggestion && (
          <div className={classes.timeSuggestedMessage}>
            <h3>
              Opponent proposes to move the time of games to{" "}
              {moment(timeSuggestion).format("DD.MM.YYYY HH:mm")}
            </h3>
            <h3>
              Accept the time change or offer an alternative by clicking the
              button above
            </h3>
            <Button7x
              className={classes.button}
